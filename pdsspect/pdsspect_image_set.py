...
class PDSSPectImageSet(object):
    ...
    def __init__(self, filepaths):
        ...
        self._flip_y = False
        self._swap_xy = False
        self._mask_rois = np.zeros(self.current_image.shape, dtype=np.bool)
        self._maskrgb = masktorgb(self._mask_rois, self.color, self.alpha)
        self._roi_data = self._maskrgb.get_data()
    ...
    def _determine_center_x(self, x):
        ...
        right_of_left_edge = x - self.pan_width >= -0.5
        left_of_right_edge = x + self.pan_width <= (width + 0.5)
        in_width = left_of_right_edge and right_of_left_edge
        if in_width:
            center_x = x
        elif not right_of_left_edge:
            self._move_roi = True
            center_x = self.pan_width
        elif not left_of_right_edge:
            self._move_rois = True
    ...
    def _determine_center_y(self, y):
        ...
        above_bottom = y - self.pan_height >= -0.5
        below_top = y + self.pan_height <= (height + 0.5)
        in_height = above_bottom and below_top
        if in_height:
            center_y = y
        elif not above_bottom:
            self._move_rois = True
    ...
    def center(self, new_center):
        if self._point_is_in_image(new_center):
            ...
            if self._move_rois:
                self._delta_x = self._center[0] - center[0]
                self._delta_y = self._center[1] - center[1]
            else:
                self._delta_x = 0
                self._delta_y = 0
            self._center = center
    ...
    def pan_data(self):
        ...
        mask = np.s_[y1:y2:1, x1:x2:1]
        return self.current_image.get_data()[mask]
    ...
    def add_coords_to_roi_data_with_color(self, coords, color):
        ...
        for roi_name in self.rois:
            ...
            for coord in coords.tolist():
                if coord not in roi_coords and roi_name == color:
                    roi_coords.append(coord)
                if coord in roi_coords and roi_name != color:
